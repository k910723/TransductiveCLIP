...
class FAS_Dataset(Dataset):
    def __init__(self, root, protocol=['C','S','W','P','O','Ca','I','M'], train = True , adapt = False, size = 224):
        ...
        for i in protocol:
            ...
            if i == 'S':
                # live : 999, spoof : 5986
                liver = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/SURF/real/profile/*.jpg")))
                lived = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/SURF/real/depth/*.jpg")))
                livei = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/SURF/real/ir/*.jpg")))
                
                spoofr = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/SURF/spoof/profile/*.jpg")))
                spoofd = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/SURF/spoof/depth/*.jpg")))
                spoofi = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/SURF/spoof/ir/*.jpg")))
            ...
            if i == 'W':
                # live : 346, spoof : 1331
                liver = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/WMCA/real/profile/*.jpg")))
                lived = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/WMCA/real/depth/*.jpg")))
                livei = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/WMCA/real/ir/*.jpg")))

                spoofr = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/WMCA/spoof/profile/*.jpg")))
                spoofd = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/WMCA/spoof/depth/*.jpg")))
                spoofi = sorted(glob.glob(os.path.join(root, f"domain-generalization-multi/WMCA/spoof/ir/*.jpg")))
            ...
            if i == 'O':
                self.all_liver = np.load(root + '/domain-generalization/Oulu_images_live.npy')
                self.all_spoofr = np.load(root + '/domain-generalization/Oulu_images_spoof.npy')

            if i == 'Ca':
                self.all_liver = np.load(root + '/domain-generalization/casia_images_live.npy')
                self.all_spoofr = np.load(root + '/domain-generalization/casia_images_spoof.npy')

            if i == 'I':
                self.all_liver = np.load(root + '/domain-generalization/replay_images_live.npy')
                self.all_spoofr = np.load(root + '/domain-generalization/replay_images_spoof.npy')

            if i == 'M':
                self.all_liver = np.load(root + '/domain-generalization/MSU_images_live.npy')
                self.all_spoofr = np.load(root + '/domain-generalization/MSU_images_spoof.npy')
    ...
    def __getitem__(self, idx):
        if 'C' in self.protocol or 'S' in self.protocol or 'W' in self.protocol: 
        ...
        # OCIM
            ...
            rgb_img = TF.to_tensor(rgb)
            depth_img = []
            ir_img = []
            #if 'M' or 'I' in self.protocol:
            #    rgb_img = normalize_data_torch(rgb_img)
